...
/**
 * Paso 4 de la cascada: el dictamen del modelo sobre un candidato.
 *
 * Se extrajo de `api/agentes/analyze/route.ts` cuando la cascada pasó a poder
 * ejecutarse también desde el cron, sin sesión. La ruta sigue siendo la dueña
 * de la autenticación; el prompt, la precedencia del precio objetivo y el
 * saneado del stop viven aquí, para que el trabajo programado y el botón de la
 * interfaz no puedan divergir en la única decisión que no es determinista.
 */
...
/**
 * Error con el código HTTP que le corresponde.
 *
 * La ruta lo traduce a una respuesta; la cascada solo lo registra y sigue con
 * el siguiente candidato. Sin esto, extraer la lógica de la ruta habría
...
/** Convicción que se asume cuando el modelo no la devuelve: no aprueba. */
export const CONVICCION_POR_DEFECTO = 5
/** Mínimo para que un candidato pase el paso 4. */
export const CONVICCION_MINIMA = 7

/**
 * ¿Aprueba este dictamen?
 *
 * La dirección se comprueba además de la convicción. El prompt ya no trae
 * "COMPRA" prerrellenada, así que el modelo puede devolver NEUTRO o VENTA con
 * convicción alta; esta cartera es solo larga (`positions.ts` calcula el P&L
 * como `valorActual - TICKET`), de modo que guardar un pick bajista invertiría
 * el signo del rendimiento.
 */
export function apruebaAnalisis(r: Record<string, unknown>): {
  pass: boolean
  conviction: number
  direction: string
  motivo: string
} {
  const conviction = (r.conviction as number) ?? CONVICCION_POR_DEFECTO
  const direction = ((r.direction as string) ?? 'COMPRA').toUpperCase()
  const convictionOk = conviction >= CONVICCION_MINIMA
  const directionOk = direction === 'COMPRA'
  const motivo = !convictionOk
    ? ` → conviction insuficiente (<${CONVICCION_MINIMA})`
    : !directionOk ? ` → dirección ${direction}, no es compra` : ''
  return { pass: convictionOk && directionOk, conviction, direction, motivo }
}

export async function analizarTicker(body: AnalyzeInput): Promise<AnalysisResult> {
  const apiKey = process.env.OPENROUTER_API_KEY
  if (!apiKey) throw new AnalisisError('OPENROUTER_API_KEY no configurada', 500)
  ...
  // cifra inventada, que es justo lo que este módulo no debe hacer.
  if (!Number.isFinite(lastPrice) || lastPrice <= 0)
    throw new AnalisisError('lastPrice (precio del subyacente) ausente o no válido', 400)
  ...
  // mezclada 60/40 con una EWMA de 30 (ver `lib/agentes/signals.ts`). Llamarla
...
Responde SOLO con JSON válido (sin markdown, sin explicación):
{
  ...
  if (!res.ok) {
    ...
    throw new AnalisisError(`OpenRouter ${res.status}: ${err}`, 500)
  }
  ...
  try {
    ...
    throw new AnalisisError('AI JSON inválido', 500, raw)
  }
