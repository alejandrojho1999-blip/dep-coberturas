...
function TickerCard({ t }: { t: TickerStage }) {
  ...
  const typeColor = t.optionType === 'CALL' ? '#4ade80' : t.optionType === 'PUT' ? '#f87171' : '#64748b'
...
export default function AgenteGamma() {
  ...
  const funnelStages = [
    { label: 'Candidatos', count: tickers.length,                                color: '#fbbf24' },
    ...
    { label: 'Calidad',    count: tickers.filter(t => t.step4 === 'pass').length, color: '#fb923c' },
    { label: 'IA',         count: tickers.filter(t => t.step5 === 'pass').length, color: '#f472b6' },
    ...
    <div className="space-y-4">
          ...
          return (
            <div key={i} className="rounded-xl border border-[#1e1e2e] bg-[#12121a] p-3 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-0.5 h-full" style={{
                background: s.phase === 'done' ? '#a78bfa' : s.phase === 'running' ? '#fbbf24' : s.phase === 'error' ? '#f87171' : '#1e1e2e'
              }} />
              ...
              <div className="pl-2 flex items-start gap-1.5">
                <Icon size={12} style={{ color: s.phase === 'done' ? '#a78bfa' : s.phase === 'running' ? '#fbbf24' : '#475569' }} className="mt-0.5 shrink-0" />
                <div>
      ...
      {tickers.length > 0 && (
        <div className="rounded-xl border border-[#1e1e2e] bg-[#12121a] p-4">
          <p className="text-[9px] font-mono text-[#475569] mb-3">EMBUDO DE SELECCIÓN — OPCIONES DIRECCIONALES</p>
      ...
      {summary && summary.created > 0 && (() => {
        ...
        return (
          <div className="rounded-xl border p-4 space-y-3" style={{ background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.25)' }}>
            <p className="text-xs font-bold font-mono" style={{ color: '#a78bfa' }}>
              ✅ {summary.created} opción(es) aprobaron los 5 filtros
            </p>
            <div className="space-y-1.5">
              {approved.map(t => {
                const typeColor = t.optionType === 'CALL' ? '#4ade80' : '#f87171'
                return (
                  <div key={t.ticker} className="flex items-center gap-3 py-1.5 px-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)' }}>
                    ...
                    <span className="text-xs text-[#94a3b8] flex-1">${t.strike} · {t.dte}d · Δ{t.delta?.toFixed(2)}</span>
                    <span className="text-[9px] font-mono px-1.5 py-0.5 rounded shrink-0" style={{ color: '#a78bfa', border: '1px solid rgba(167,139,250,0.3)', background: 'rgba(167,139,250,0.08)' }}>
                      {t.conviction}/10
                    </span>
                  </div>
                )
